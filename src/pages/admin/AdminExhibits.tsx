...
const AdminExhibits = () => {
  ...
  const handleSubmit = async ( e: React.FormEvent ) => {
    ...
    try {
      ...
      // backend expects `title` and `content` for posts
      submitData.append( 'title', formData.name );
      submitData.append( 'content', formData.description );
      // optional fields - PostsSchema doesn't currently store these, but we include them
      submitData.append( 'location', formData.location );
      submitData.append( 'category', formData.category );

      // append selected image files (field name must match multer field: "image")
      if ( selectedFiles && selectedFiles.length > 0 ) {
        if ( editingExhibit ) {
          // update endpoint expects single file (upload.single("image"))
          submitData.append( 'image', selectedFiles[ 0 ] );
        } else {
          // create endpoint accepts multiple files (upload.array("image"))
