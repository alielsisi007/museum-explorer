...
const Booking = () => {
  ...
  const handleProceedToPayment = () => {
    if ( !isAuthenticated ) {
      toast( { title: 'Login Required', description: 'Please login to book tickets.', variant: 'destructive' } );
      navigate( '/login', { state: { from: '/booking' } } );
      return;
    }
    if ( !date ) {
      toast( { title: 'Select a Date', description: 'Please select a visit date.', variant: 'destructive' } );
      return;
    }
    if ( totalTickets === 0 ) {
  ...
  const handleConfirmPayment = async () => {
    ...
    try {
      ...
      toast( { title: 'Booking Failed', description: 'Error processing your booking.', variant: 'destructive' } );
    } finally {
      setIsLoading( false );
    }
  };
    ...
    <Layout>
      <section className="bg-primary py-16 md:py-24">
        <div className="container-museum">
          <motion.div initial={ { opacity: 0, y: 20 } } animate={ { opacity: 1, y: 0 } } className="text-center">
            <h1 className="text-4xl md:text-5xl font-serif font-bold text-primary-foreground mb-4">Book Your Visit</h1>
            <p className="text-primary-foreground/80 max-w-2xl mx-auto text-lg">Select your preferred date and tickets to plan your museum experience.</p>
          </motion.div>
        </div>
      </section>
      ...
      <section className="section-padding bg-background">
        <div className="container-museum max-w-4xl">
          ...
          <div className="flex items-center justify-center mb-12">
            { [ 'Select', 'Payment', 'Confirmation' ].map( ( label, index ) => (
              <div key={ label } className="flex items-center">
                ...
                >
                  { ( step === 'payment' && index === 0 ) || ( step === 'confirmation' && index < 2 ) ? <Check className="w-5 h-5" /> : index + 1 }
                </div>
          ...
          { step === 'select' && (
            <motion.div initial={ { opacity: 0, x: 20 } } animate={ { opacity: 1, x: 0 } } className="grid md:grid-cols-2 gap-8">
              {/* Date Selection */ }
              ...
              <div className="bg-card p-6 rounded-xl shadow-soft">
                ...
                <div className="space-y-4">
                  { ticketTypes.map( ( type ) => (
                    <div key={ type._id } className="flex items-center justify-between p-3 bg-secondary/50 rounded-lg">
                      <div>
                      ...
                      <div className="flex items-center gap-3">
                        <Button variant="outline" size="icon" className="h-8 w-8" onClick={ () => updateQuantity( type._id, -1 ) } disabled={ ( quantities[ type._id ] || 0 ) === 0 }>
                          <Minus className="w-4 h-4" />
                        </Button>
                        <span className="w-8 text-center font-medium">{ quantities[ type._id ] || 0 }</span>
                        <Button variant="outline" size="icon" className="h-8 w-8" onClick={ () => updateQuantity( type._id, 1 ) }>
              ...
              <div className="md:col-span-2 bg-primary p-6 rounded-xl">
                <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
                  ...
                  <Button className="btn-gold w-full sm:w-auto" onClick={ handleProceedToPayment } disabled={ !date || totalTickets === 0 }>
                    Proceed to Payment
                  </Button>
                </div>
              </div>
            ...
            <motion.div initial={ { opacity: 0, x: 20 } } animate={ { opacity: 1, x: 0 } } className="max-w-xl mx-auto">
              <div className="bg-card p-8 rounded-xl shadow-soft">
                ...
                <div className="flex gap-4">
                  ...
                  <Button className="btn-gold flex-1" onClick={ handleConfirmPayment } disabled={ isLoading }>
